    ...
    <div class="flex flex-col md:flex-row min-h-screen bg-gray-50 pb-24 md:pb-0">
        ...
        <main class="flex-1 py-10">
            <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
                <a href="{{ route('pelanggan.pengaduan.index') }}" class="text-xs text-indigo-600 hover:underline">&larr; Kembali</a>
                ...
                <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-200 mt-4">
                    ...
                    <form action="{{ route('pelanggan.pengaduan.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                        @csrf
                        <div>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <input type="text" name="latitude" id="latitude" placeholder="-6.917464" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm text-xs focus:border-indigo-500 focus:ring-indigo-500" value="{{ $pelanggan->latitude ?? '' }}">
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-gray-500 uppercase">Longitude</label>
                                <input type="text" name="longitude" id="longitude" placeholder="107.619123" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm text-xs focus:border-indigo-500 focus:ring-indigo-500" value="{{ $pelanggan->longitude ?? '' }}">
                            </div>
                        </div>
                        ...
                        <div>
                            <label class="block text-xs font-bold text-gray-500 uppercase">Foto Bukti (opsional)</label>
                            <input type="file" name="foto_bukti" accept="image/*" class="mt-1 block w-full text-xs text-gray-500 file:mr-4 file:py-1.5 file:px-3 file:rounded-md file:border-0 file:text-xs file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100">
                        </div>
