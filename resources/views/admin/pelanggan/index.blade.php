    ...
    <div class="flex flex-col md:flex-row min-h-screen bg-gray-100 pb-24 md:pb-0">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="p-6 bg-white shadow-sm sm:rounded-xl border border-gray-100">
                <h3 class="text-base font-bold text-gray-900 mb-4 border-b pb-2">Registrasi Pelanggan Baru</h3>
                ...
                <form action="{{ route('admin.pelanggan.store') }}" method="POST" class="space-y-4">
                    ...
                    <div class="border-t pt-4 mt-2">
                        <div class="flex items-center justify-between mb-2">
                            <label class="block font-semibold text-sm text-gray-700">📍 Titik Koordinat Lokasi (Klik Peta)</label>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <x-text-input id="latitude" name="latitude" type="text" class="mt-1 block w-full bg-gray-50" readonly placeholder="Klik pada peta..." />
                            </div>
                            <div>
                                <x-input-label for="longitude" value="Longitude" />
                                <x-text-input id="longitude" name="longitude" type="text" class="mt-1 block w-full bg-gray-50" readonly placeholder="Klik pada peta..." />
                            </div>
                        </div>
                    </div>
            ...
            <div class="p-6 bg-white shadow-sm sm:rounded-xl border border-gray-100">
                <h3 class="text-base font-bold text-gray-900 mb-4 border-b pb-2">📋 Daftar Pelanggan Terregistrasi</h3>
                
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse text-sm">
                        <thead>
                            <tr class="border-b bg-gray-50 text-gray-600 text-xs uppercase font-semibold">
                                <th class="p-3">No Pelanggan</th>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse($dataPelanggan as $plg)
                                <tr class="hover:bg-gray-50 transition">
                                    <td class="p-3 font-bold text-indigo-600 font-mono">{{ $plg->no_pelanggan }}</td>
                                    ...
                                    <td class="p-3 text-center">
                                        <div class="flex items-center justify-center gap-2">
                                            <a href="{{ route('admin.pelanggan.edit', $plg->id) }}" class="text-xs text-indigo-600 hover:text-indigo-800 hover:underline font-semibold">
                                                Edit
                                            </a>
                                            <form action="{{ route('admin.pelanggan.destroy', $plg->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus pelanggan ini beserta akun usernya?')">
                                                @csrf
                                                @method('DELETE')
                            ...
                            @empty
                                <tr>
                                    <td colspan="8" class="p-6 text-center text-gray-400">Belum ada data pelanggan yang terdaftar.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
