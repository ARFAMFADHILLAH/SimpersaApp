        ...
        <main class="flex-1 py-12 px-4 sm:px-6 lg:px-8">
            <div class="max-w-7xl mx-auto space-y-6">
                ...
                <div class="flex gap-3">
                    <a href="{{ route('admin.operasional.rekap-volume') }}" class="px-4 py-2 bg-cyan-600 text-white rounded-lg hover:bg-cyan-700 text-sm">Rekap Volume Harian/Bulanan</a>
                    <a href="{{ route('admin.operasional.jadwal-rute') }}" class="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 text-sm">Jadwal Rute & Penugasan</a>
                </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th class="px-4 py-2 text-left text-xs font-medium text-gray-500">Warga</th>
                            ...
                            <tbody class="divide-y divide-gray-200">
                                @forelse($pengangkutanHariIni as $angkut)
                                    <tr>
                                        <td class="px-4 py-2 text-sm">{{ $angkut->warga?->user?->name ?? '-' }}</td>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        @forelse($rutes as $rute)
                            <div class="p-4 bg-gray-50 rounded-lg border border-gray-200">
                                ...
                                <p class="text-sm text-gray-500">{{ $rute->hari_angkut ?? '-' }} &middot; {{ $rute->warga_count }} warga</p>
                            </div>
                        @empty
