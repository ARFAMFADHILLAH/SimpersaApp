        ...
        <main class="flex-1 py-12 px-4 sm:px-6 lg:px-8">
            <div class="max-w-7xl mx-auto space-y-6">
                <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                    <h3 class="text-lg font-semibold text-gray-900 mb-2">Detail Pelanggan</h3>
                    <dl class="grid grid-cols-2 gap-4 text-sm">
                        <div><dt class="text-gray-500">No Pelanggan</dt><dd class="font-medium">{{ $pelanggan->no_pelanggan }}</dd></div>
                        <div><dt class="text-gray-500">Nama</dt><dd class="font-medium">{{ $pelanggan->user?->name ?? '-' }}</dd></div>
                        <div><dt class="text-gray-500">Email</dt><dd>{{ $pelanggan->user?->email ?? '-' }}</dd></div>
                        <div><dt class="text-gray-500">No HP</dt><dd>{{ $pelanggan->no_hp ?? '-' }}</dd></div>
                        <div><dt class="text-gray-500">Alamat</dt><dd class="col-span-2">{{ $pelanggan->alamat_lengkap ?? '-' }}</dd></div>
                        <div><dt class="text-gray-500">Rute</dt><dd>{{ $pelanggan->rute?->nama_rute ?? '-' }}</dd></div>
                        <div><dt class="text-gray-500">Wilayah</dt><dd>{{ $pelanggan->wilayah?->nama_wilayah ?? '-' }}</dd></div>
                    </dl>
                </div>
