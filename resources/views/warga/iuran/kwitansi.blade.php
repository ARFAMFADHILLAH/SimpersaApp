...
<head>
    ...
    <style>
        ...
        .bukti { max-width: 400px; margin: 24px auto 0; border: 1px solid #ddd; padding: 16px; text-align: center; }
        .bukti img { max-width: 100%; max-height: 320px; }
        .bukti .label { font-size: 11px; color: #666; margin-bottom: 8px; }
...
<body>
    <div class="kwitansi">
        ...
        <table>
            ...
            <tr><td class="label">No Warga</td><td>: {{ $warga->no_warga }}</td></tr>
            <tr><td class="label">Nama</td><td>: {{ $warga->user->name }}</td></tr>
    ...
    @if($iuran->bukti_pembayaran)
        <div class="bukti">
            <div class="label">Lampiran Bukti Pembayaran</div>
            <img src="{{ \Storage::url($iuran->bukti_pembayaran) }}" alt="Bukti Pembayaran">
        </div>
    @endif
