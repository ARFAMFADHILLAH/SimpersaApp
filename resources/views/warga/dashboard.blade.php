                ...
                @endif
                    ...
                    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-200 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                        <div>
                            ...
                            <p class="text-sm text-gray-500 mt-0.5">Rute: {{ $warga->rute->nama_rute ?? 'Belum Ditentukan' }} &middot; Wilayah: {{ $warga->wilayahPelayanan->nama_wilayah ?? '-' }}</p>
                        </div>
                        <div class="flex items-center gap-2 bg-green-50 text-green-700 px-4 py-2 rounded-lg border border-green-200">
                            <span class="text-xs font-bold">{{ $warga->no_hp ?? '-' }}</span>
                        </div>
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-200 flex flex-col justify-between">
                        <div>
                            ...
                            @if($iuranDiproses)
                                <p class="text-xs text-amber-600 font-medium mt-1">Menunggu verifikasi bendahara ({{ $iuranDiproses->bulan_tagihan }})</p>
                            @elseif($tagihanBulanIni)
                        ...
                        <a href="{{ route('warga.iuran.index') }}" class="mt-4 block w-full text-center bg-green-600 hover:bg-green-700 disabled:bg-gray-200 text-white font-semibold text-xs py-2.5 px-4 rounded-lg transition shadow-sm">
                            {{ $iuranDiproses ? 'Cek Status Pembayaran' : 'Bayar Sekarang' }}
                        </a>
                    </div>

                    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                        <div class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Alamat Terdaftar</div>
                        <p class="text-xs font-medium text-gray-800 mt-3 leading-relaxed">{{ $warga->alamat_lengkap ?? 'Belum diinput.' }}</p>
                        <div class="text-[10px] text-gray-400 mt-2 font-mono">
                            GPS: {{ $warga->latitude ?? '-' }}, {{ $warga->longitude ?? '-' }}
                        </div>
                    </div>
                    ...
                    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                        ...
                        <a href="{{ route('warga.pengaduan.index') }}" class="text-xs text-green-600 hover:underline mt-2 inline-block">Lihat Semua &rarr;</a>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                                ...
                                <tbody class="divide-y divide-gray-100 text-gray-700">
                                    @forelse($riwayatIuran as $iuran)
                                        <tr class="hover:bg-gray-50">
                                            ...
                                            <td class="px-6 py-3 font-bold {{ $iuran->status_pembayaran == 'Lunas' ? 'text-green-600' : ($iuran->status_pembayaran == 'Sedang Diproses' ? 'text-amber-600' : 'text-red-500') }}">
                        ...
                        <div class="p-3 border-t bg-gray-50 text-right">
                            <a href="{{ route('warga.iuran.index') }}" class="text-xs text-green-600 hover:underline">Lihat semua &rarr;</a>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                        ...
                        <div class="p-3 border-t bg-gray-50 text-right">
                            <a href="{{ route('warga.profile.riwayat') }}" class="text-xs text-green-600 hover:underline">Lihat semua &rarr;</a>
                        </div>
                    </div>
                </div>
